...
const StudentRegisterQuizFilter = ({
  registerQuizList,
  onRegisterQuizFilter,
}) => {
  // State for filters
  ...
  const filterRegisterQuizzes = () => {
    let filteredRegisterQuizzes = [...registerQuizList];

    // Filter by quiz type
    if (selectedType !== null) {
      filteredRegisterQuizzes = filteredRegisterQuizzes.filter(
        (registerQuiz) => registerQuiz.quiz.type === selectedType
      );
    }
    ...
    if (selectedSubject !== "all") {
      filteredRegisterQuizzes = filteredRegisterQuizzes.filter(
        (registerQuiz) => registerQuiz.quiz.type === selectedSubject
      );
    }

    // Order quizzes
    if (orderBy !== null) {
      filteredRegisterQuizzes.sort((a, b) => a.id < b.id);
      // Assuming 'name' is the property for exam name
    } else {
      filteredRegisterQuizzes.sort((a, b) =>
        a.quiz.title.localeCompare(b.quiz.title)
      );
    }

    filteredRegisterQuizzes = filteredRegisterQuizzes.filter((registerQuiz) =>
      registerQuiz.quiz.title.includes(searchTitle)
    );

    onRegisterQuizFilter(filteredRegisterQuizzes);
  };
  useEffect(() => {
    filterRegisterQuizzes();
  }, [selectedType, orderBy, selectedSubject, searchTitle, registerQuizList]);
...
export default StudentRegisterQuizFilter;
