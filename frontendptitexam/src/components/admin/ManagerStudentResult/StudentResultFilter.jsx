...
const StudentResultFilter = ({
  participantAttemptList,
  onParticipantAttemptFilter,
}) => {
  // State for filters
  ...
  const filterParticipantAttempts = () => {
    let filteredParticipantAttempts = [...participantAttemptList];

    // Filter by quiz type
    if (selectedType !== null) {
      filteredParticipantAttempts = filteredParticipantAttempts.filter(
        (participantAttempt) =>
          participantAttempt.registerQuiz.quiz.type === selectedType
      );
    }
    ...
    if (selectedSubject !== "all") {
      filteredParticipantAttempts = filteredParticipantAttempts.filter(
        (participantAttempt) =>
          participantAttempt.registerQuiz.quiz.type === selectedSubject
      );
    }

    // Order quizzes
    if (orderBy !== null) {
      filteredParticipantAttempts.sort((a, b) => a.id < b.id);
      // Assuming 'name' is the property for exam name
    } else {
      filteredParticipantAttempts.sort((a, b) =>
    ...
    filteredParticipantAttempts = filteredParticipantAttempts.filter(
      (participantAttempt) =>
        participantAttempt.registerQuiz.quiz.title.includes(searchTitle)
    );

    onParticipantAttemptFilter(filteredParticipantAttempts);
  };
  useEffect(() => {
    filterParticipantAttempts();
  }, [
    selectedType,
    orderBy,
    selectedSubject,
